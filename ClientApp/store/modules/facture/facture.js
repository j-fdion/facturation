...
const load_items_url = '/api/Gestion/Factures'
const save_item_url = '/api/Gestion/SaveFacture'
const delete_item_url = '/api/Gestion/DeleteFacture'
...
const mutations = {
    ...
    SET_EDITED_ITEM: (state, { payload }) => {
        ...
        state.entreprises = [state.editedItem.entreprise]
        state.sessions.push(...state.editedItem.sessions)
    },
...
const actions = ({
    ...
    SEARCH_SESSIONS: function ({ commit }, { searchTerm }) {
        axios.get('/api/Gestion/SearchSessions', {
            params: {
                searchTerm
            }
        }).then(function (response) {
            commit(SET_SESSIONS, { payload: response.data });
        }, (err) => {
            console.log(err)
        })
    }
