...
const load_items_url = '/api/Gestion/Accessoires'
const save_item_url = '/api/Gestion/SaveAccessoire'
const delete_item_url = '/api/Gestion/DeleteAccessoire'

export const SEARCH_TYPE_ACCESSOIRES = 'SEARCH_TYPE_ACCESSOIRES'
export const SET_TYPE_ACCESSOIRES = 'SET_TYPE_ACCESSOIRES'
export const MERGE_EDIT_TO_ITEM = 'MERGE_EDIT_TO_ITEM'
export const typeAccessoires = 'typeAccessoires'
...
const mutations = {
    ...
    SET_EDITED_ITEM: (state, { payload }) => {
        ...
        state.typeAccessoires = [state.editedItem.typeAccessoire]
    },
    SET_EDITED_ITEM_DEFAULT: (state) => {
        state.editedIndex = -1
        state.editedItem = Object.assign({}, state.defaultItem);
        state.typeAccessoires = []
    },
    SET_TYPE_ACCESSOIRES: (state, { payload }) => {
        state.typeAccessoires = payload
    },
...
const actions = ({
    ...
    SEARCH_TYPE_ACCESSOIRES: function ({ commit }, { searchTerm }) {
        axios.get('/api/Gestion/SearchTypeAccessoires', {
            params: {
                searchTerm
            }
        }).then(function (response) {
            commit(SET_TYPE_ACCESSOIRES, { payload: response.data });
        }, (err) => {
            console.log(err)
        })
    }
