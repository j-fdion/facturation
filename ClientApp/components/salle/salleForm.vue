<template>
    ...
    <v-dialog v-else-if="deleteDialog" v-model="deleteDialog" max-width="700px">
        <v-card>
            <v-card-title>
                <span class="headline">Supprimer une salle</span>
            </v-card-title>
...
<script>
    ...
    const { mapState, mapActions, mapGetters, mapMutations } = createNamespacedHelpers('salle')

    import * as module from '../../store/modules/salle/salle'
    ...
    export default {
        ...
        computed: {
            formTitle() {
                return this.editedIndex === -1 ? "Ajouter une salle" : "Modifier une salle"
            },
            dialog: {
                get() {
                    return this.$store.state.salle.dialog;
                },
                set(value) {
            ...
            deleteDialog: {
                get() {
                    return this.$store.state.salle.deleteDialog;
                },
                set(value) {
            ...
            nom: {
                get() {
                    return this.$store.state.salle.editedItem.nom;
                },
                set(value) {
